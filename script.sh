...
# Start the server
python3 $PROJECT_DIR/Server.py $num_users $num_assisting_nodes $commitment $printAggOutput $bandwidth_mode &
...
for ((i=1; i<=$num_assisting_nodes; i++)); do
    python3 $PROJECT_DIR/AssistingNodeCode.py $i $num_users $num_assisting_nodes $commitment $bandwidth_mode &
    sleep 0.01
done

# Start user processes
for ((i=1; i<=$num_users; i++)); do
    python3 $PROJECT_DIR/User.py $i $num_users $num_assisting_nodes $commitment $bandwidth_mode &
